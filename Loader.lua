local library = {
	...
	colors = {
		main = Color3.fromHex(255, 199, 41),
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Shadow Hub Premium Scripts")
							if not fold then
								makefolder("./Shadow Hub Premium Scripts")
							end
							fold = isfolder("./Shadow Hub Premium Scripts/Themes")
							if not fold then
								makefolder("./Shadow Hub Premium Scripts/Themes")
							end
							fold = isfolder("./Shadow Hub Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Shadow Hub Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Shadow Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Shadow Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Shadow Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
